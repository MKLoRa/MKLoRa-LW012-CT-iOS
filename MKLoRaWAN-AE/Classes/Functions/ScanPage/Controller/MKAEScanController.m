...
static NSString *const localPasswordKey = @"mk_ae_passwordKey";

static CGFloat const searchButtonHeight = 40.f;

static NSTimeInterval const kRefreshInterval = 0.5f;

@interface MKAEScanController ()<UITableViewDelegate,
UITableViewDataSource,
mk_ae_centralManagerScanDelegate,
MKAEScanPageCellDelegate,
MKSearchButtonDelegate,
MKAETabBarControllerDelegate>
...
@implementation MKAEScanController

- (void)dealloc {
    NSLog(@"MKAEScanController销毁");
    [[NSNotificationCenter defaultCenter] removeObserver:self];
    //移除runloop的监听
    CFRunLoopRemoveObserver(CFRunLoopGetCurrent(), self.observerRef, kCFRunLoopCommonModes);
    [[MKAECentralManager shared] stopScan];
    [MKAECentralManager removeFromCentralList];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    //记录下来当前设备类型
    [[NSUserDefaults standardUserDefaults] setObject:@(self.deviceType) forKey:@"ae_deviceType"];
    [self loadSubViews];
    self.searchButton.dataModel = self.buttonModel;
    [self runloopObserver];
    [MKAECentralManager shared].delegate = self;
...
- (void)rightButtonMethod {
    UIViewController *vc = [[CTMediator sharedInstance] CTMediator_LORAWAN_AE_AboutPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    MKAEScanPageCell *cell = [MKAEScanPageCell initCellWithTableView:self.tableView];
...
#pragma mark - mk_ae_centralManagerScanDelegate
- (void)mk_ae_receiveDevice:(NSDictionary *)deviceModel {
    MKAEScanPageModel *model = [MKAEScanPageModel mk_modelWithJSON:deviceModel];
...
#pragma mark - MKAEScanPageCellDelegate
- (void)ae_scanCellConnectButtonPressed:(NSInteger)index {
    [self connectDeviceWithModel:self.dataList[index]];
}

#pragma mark - MKAETabBarControllerDelegate
- (void)mk_ae_needResetScanDelegate:(BOOL)need {
    if (need) {
        [MKAECentralManager shared].delegate = self;
    }
...
- (void)scanTimerRun{
    ...
    [[MKAECentralManager shared] startScan];
...
- (void)updateDataWithScanModel:(MKAEScanPageModel *)scanDataModel{
 ...
 */
- (void)filterTrackerDataWithSearchName:(MKAEScanPageModel *)scanDataModel{
    if (scanDataModel.rssi < self.buttonModel.searchRssi) {
        return;
    }
...
- (void)processTrackerData:(MKAEScanPageModel *)scanDataModel{
    //查看数据源中是否已经存在相关设备
 ...
 */
- (void)dataNoExistDataSource:(MKAEScanPageModel *)scanDataModel{
 ...
 */
- (void)dataExistDataSource:(MKAEScanPageModel *)scanDataModel {
    NSInteger currentIndex = 0;
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        MKAEScanPageModel *dataModel = self.dataList[i];
        if ([dataModel.macAddress isEqualToString:scanDataModel.macAddress]) {
            currentIndex = i;
            break;
        }
    }
    MKAEScanPageModel *dataModel = self.dataList[currentIndex];
...
#pragma mark - 连接部分
- (void)connectDeviceWithModel:(MKAEScanPageModel *)scanDataModel {
    //停止扫描
    [self.refreshIcon.layer removeAnimationForKey:@"mk_refreshAnimationKey"];
    [[MKAECentralManager shared] stopScan];
    if (self.scanTimer) {
    ...
    [alertView showAlertWithTitle:@"Enter password" message:msg notificationName:@"mk_ae_needDismissAlert"];
}

- (void)connectDeviceWithDataModel:(MKAEScanPageModel *)scanDataModel {
    ...
    [[MKAEConnectModel shared] connectDevice:scanDataModel.peripheral password:(scanDataModel.needPassword ? password : @"") deviceType:scanDataModel.deviceType sucBlock:^{
...
- (void)configParams {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"ae_readRecordDataDayNumKey"];
    //读取设备存储的扫描信息页面，显示的设备总共存储了多少条的扫描数据
    [[NSUserDefaults standardUserDefaults] setObject:@"" forKey:@"ae_recordDataTotalSumKey"];
    
    [MKAEDatabaseManager clearDataTable];
    [MKAEDatabaseManager initDataBase];
}

- (void)pushTabBarPage {
    MKAETabBarController *vc = [[MKAETabBarController alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEScanController", @"ae_scanRightAboutIcon.png") forState:UIControlStateNormal];
...
- (UIImageView *)refreshIcon {
    if (!_refreshIcon) {
        ...
        _refreshIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEScanController", @"ae_scan_refreshIcon.png");
    }
    return _refreshIcon;
}
