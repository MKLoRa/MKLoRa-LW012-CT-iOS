...
@implementation MKAEScanPageCell

+ (MKAEScanPageCell *)initCellWithTableView:(UITableView *)tableView {
    MKAEScanPageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAEScanPageCellIdenty"];
    if (!cell) {
        cell = [[MKAEScanPageCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAEScanPageCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ae_scanCellConnectButtonPressed:)]) {
        [self.delegate ae_scanCellConnectButtonPressed:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKAEScanPageModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAEScanPageModel.class]) {
        return;
    }
    //顶部
...
- (UIImageView *)rssiIcon {
    if (!_rssiIcon) {
        ...
        _rssiIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEScanPageCell", @"ae_scan_rssiIcon.png");
    }
    return _rssiIcon;
}
...
- (UIImageView *)batteryIcon {
    if (!_batteryIcon) {
        ...
        _batteryIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEScanPageCell", @"ae_scan_batteryIcon.png");
    }
    return _batteryIcon;
}
