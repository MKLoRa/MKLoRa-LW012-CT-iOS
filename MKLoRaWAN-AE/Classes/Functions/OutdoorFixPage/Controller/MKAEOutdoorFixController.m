...
@interface MKAEOutdoorFixController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEOutdoorFixModel *dataModel;

@end

@implementation MKAEOutdoorFixController

- (void)dealloc {
    NSLog(@"MKAEOutdoorFixController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEOutdoorFixController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEOutdoorFixModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEOutdoorFixModel alloc] init];
    }
    return _dataModel;
}
