...
@interface MKAETimeSegmentedController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKAETimeSegmentedAddCellDelegate,
MKAETimeSegmentedCellDelegate>
...
@property (nonatomic, strong)MKAETimeSegmentedModel *dataModel;

@end

@implementation MKAETimeSegmentedController

- (void)dealloc {
    NSLog(@"MKAETimeSegmentedController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKAETimeSegmentedAddCell *cell = [MKAETimeSegmentedAddCell initCellWithTableView:tableView];
        cell.dataModel = self.section1List[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    MKAETimeSegmentedCell *cell = [MKAETimeSegmentedCell initCellWithTableView:tableView];
...
#pragma mark - MKAETimeSegmentedAddCellDelegate
- (void)ae_timeSegmentedAddCell_addPressed {
    ...
    MKAETimeSegmentedCellModel *cellModel = [[MKAETimeSegmentedCellModel alloc] init];
...
#pragma mark - MKAETimeSegmentedCellDelegate
/**
 删除
 
 @param index 所在index
 */
- (void)ae_timeSegmentedCell_deleteButtonPressed:(NSInteger)index {
    ...
    for (NSInteger i = 0; i < self.section2List.count; i ++) {
        MKAETimeSegmentedCellModel *cellModel = self.section2List[i];
        cellModel.index = i;
    }
...
///   - start: 起始时间还是结束时间
///   - hour: 当前选择的hour
- (void)ae_timeSegmentedCell_hourButtonPressed:(NSInteger)index start:(BOOL)start hour:(NSInteger)hour {
...
- (void)ae_timeSegmentedCell_minuteGearButtonPressed:(NSInteger)index start:(BOOL)start minuteGear:(NSInteger)minuteGear {
    if (![self cellCanSelected]) {
        return;
    }
    MKAETimeSegmentedCellModel *cellModel = self.section2List[index];
...
- (BOOL)cellCanSelected{
    ...
    for (int i = 0; i < [arrCells count]; i++) {
        ...
        if ([cell isKindOfClass:MKAETimeSegmentedCell.class]) {
            MKAETimeSegmentedCell *tempCell = (MKAETimeSegmentedCell *)cell;
...
- (void)loadSection1Datas {
    MKAETimeSegmentedAddCellModel *cellModel = [[MKAETimeSegmentedAddCellModel alloc] init];
...
- (void)loadSection2Datas {
    for (NSInteger i = 0; i < self.dataModel.pointList.count; i ++) {
        MKAETimeSegmentedTimePeriodModel *tempModel = self.dataModel.pointList[i];
        MKAETimeSegmentedCellModel *cellModel = [[MKAETimeSegmentedCellModel alloc] init];
        cellModel.index = i;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAETimeSegmentedController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAETimeSegmentedModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAETimeSegmentedModel alloc] init];
    }
    return _dataModel;
}
