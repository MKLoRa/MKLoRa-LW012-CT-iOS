...
@implementation MKAETimeSegmentedCellModel
@end

@interface MKAETimeSegmentedCell ()
...
@implementation MKAETimeSegmentedCell

+ (MKAETimeSegmentedCell *)initCellWithTableView:(UITableView *)tableView {
    MKAETimeSegmentedCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAETimeSegmentedCellIdenty"];
    if (!cell) {
        cell = [[MKAETimeSegmentedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAETimeSegmentedCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ae_timeSegmentedCell_deleteButtonPressed:)]) {
        [self.delegate ae_timeSegmentedCell_deleteButtonPressed:self.dataModel.index];
    }
}
...
- (void)contentPanelTapAction {
    if (self.contentPanel.frame.origin.x == 0) {
        if ([self.delegate respondsToSelector:@selector(ae_cellTapAction)]) {
            [self.delegate ae_cellTapAction];
        }
        return;
    }
    if (self.contentPanel.frame.origin.x < 0){
...
- (void)swipeEventBeTriggered:(UISwipeGestureRecognizer *)swipeGesture{
    if ([self.delegate respondsToSelector:@selector(ae_cellResetFrame)]) {
        [self.delegate ae_cellResetFrame];
    }
...
#pragma mark - setter
- (void)setDataModel:(MKAETimeSegmentedCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAETimeSegmentedCellModel.class]) {
        return;
    }
...
- (void)hourButtonPressed:(BOOL)start {
    ...
    [pickView showPickViewWithDataList:self.hourList selectedRow:selectedRow block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ae_timeSegmentedCell_hourButtonPressed:start:hour:)]) {
            [self.delegate ae_timeSegmentedCell_hourButtonPressed:self.dataModel.index start:start hour:currentRow];
        }
    }];
}
...
- (void)miniuteButtonPressed:(BOOL)start {
    ...
    [pickView showPickViewWithDataList:self.minuteGearList selectedRow:selectedRow block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ae_timeSegmentedCell_minuteGearButtonPressed:start:minuteGear:)]) {
            [self.delegate ae_timeSegmentedCell_minuteGearButtonPressed:self.dataModel.index start:start minuteGear:currentRow];
        }
    }];
}
...
- (MKTextField *)intervalTextField {
    if (!_intervalTextField) {
        ...
        _intervalTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(ae_timeSegmentedCell_reportIntervalChanged:interval:)]) {
                [self.delegate ae_timeSegmentedCell_reportIntervalChanged:self.dataModel.index interval:text];
            }
        };
    }
