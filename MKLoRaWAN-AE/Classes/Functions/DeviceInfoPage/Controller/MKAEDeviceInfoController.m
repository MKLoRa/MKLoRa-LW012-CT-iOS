...
@interface MKAEDeviceInfoController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEDeviceInfoModel *dataModel;
...
@implementation MKAEDeviceInfoController

- (void)dealloc {
    NSLog(@"MKAEDeviceInfoController销毁");
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(deviceStartDFUProcess)
                                                 name:@"mk_ae_startDfuProcessNotification"
                                               object:nil];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 5 && indexPath.row == 0) {
        ...
        MKAEBatteryConsumptionController *vc = [[MKAEBatteryConsumptionController alloc] init];
    ...
    if (indexPath.section == 6 && indexPath.row == 0) {
        //Debugger Mode
        MKAEDebuggerController *vc = [[MKAEDebuggerController alloc] init];
...
- (void)ae_textButtonCell_buttonAction:(NSInteger)index {
    if (index == 0) {
        //DFU
        MKAEUpdateController *vc = [[MKAEUpdateController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)pushSelftestInterface {
    MKAESelftestController *vc = [[MKAESelftestController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)updateCellDatas {
    ...
    if (ValidStr(self.dataModel.firmware)) {
        MKAETextButtonCellModel *firmwareModel = self.section1List[0];
...
- (MKAEDeviceInfoModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEDeviceInfoModel alloc] init];
    }
    return _dataModel;
}
