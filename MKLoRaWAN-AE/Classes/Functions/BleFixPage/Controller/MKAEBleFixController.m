...
#import "MKAEFilterByMacController.h"
#import "MKAEFilterByAdvNameController.h"
#import "MKAEFilterByRawDataController.h"

@interface MKAEBleFixController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKNormalSliderCellDelegate,
MKAEFilterRelationshipCellDelegate>
...
@property (nonatomic, strong)MKAEBleFixDataModel *dataModel;

@end

@implementation MKAEBleFixController

- (void)dealloc {
    NSLog(@"MKAEBleFixController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 4 && indexPath.row == 0) {
        //Filter by MAC
        MKAEFilterByMacController *vc = [[MKAEFilterByMacController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 4 && indexPath.row == 1) {
        //Filter by ADV Name
        MKAEFilterByAdvNameController *vc = [[MKAEFilterByAdvNameController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 4 && indexPath.row == 2) {
        //Filter by Raw Data
        MKAEFilterByRawDataController *vc = [[MKAEFilterByRawDataController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKAEFilterRelationshipCell *cell = [MKAEFilterRelationshipCell initCellWithTableView:tableView];
...
#pragma mark - MKAEFilterRelationshipCellDelegate
- (void)ae_filterRelationshipChanged:(NSInteger)index dataListIndex:(NSInteger)dataListIndex {
    if (index == 0) {
        ...
        MKAEFilterRelationshipCellModel *cellModel = self.section1List[0];
    ...
    if (index == 1) {
        ...
        MKAEFilterRelationshipCellModel *cellModel = self.section3List[0];
        cellModel.dataListIndex = dataListIndex;
        return;
    }
}
...
- (void)loadSection1Datas {
    MKAEFilterRelationshipCellModel *cellModel1 = [[MKAEFilterRelationshipCellModel alloc] init];
    cellModel1.index = 0;
...
- (void)loadSection3Datas {
    MKAEFilterRelationshipCellModel *cellModel = [[MKAEFilterRelationshipCellModel alloc] init];
    cellModel.index = 1;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEBleFixController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEBleFixDataModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEBleFixDataModel alloc] init];
    }
    return _dataModel;
}
