...
@interface MKAEFilterByAdvNameModel ()
...
@implementation MKAEFilterByAdvNameModel
...
- (BOOL)readFilterPreciseMatch {
    ...
    [MKAEInterface ae_readFilterByAdvNamePreciseMatchWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterPreciseMatch {
    __block BOOL success = NO;
    [MKAEInterface ae_configFilterByAdvNamePreciseMatch:self.match sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readReverseFilter {
    __block BOOL success = NO;
    [MKAEInterface ae_readFilterByAdvNameReverseFilterWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReverseFilter {
    __block BOOL success = NO;
    [MKAEInterface ae_configFilterByAdvNameReverseFilter:self.filter sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readAdvNameList {
    __block BOOL success = NO;
    [MKAEInterface ae_readFilterAdvNameListWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configAdvNameList:(NSArray <NSString *>*)list {
    __block BOOL success = NO;
    [MKAEInterface ae_configFilterAdvNameList:list sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
