...
@interface MKAELoRaSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAELoRaSettingModel *dataModel;

@end

@implementation MKAELoRaSettingController

- (void)dealloc {
    NSLog(@"MKAELoRaSettingController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAELoRaSettingController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAELoRaSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAELoRaSettingModel alloc] init];
    }
    return _dataModel;
}
