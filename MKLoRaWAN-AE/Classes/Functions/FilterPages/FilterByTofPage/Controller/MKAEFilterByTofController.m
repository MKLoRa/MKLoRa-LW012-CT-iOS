...
@interface MKAEFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEFilterByTofModel *dataModel;

@end

@implementation MKAEFilterByTofController

- (void)dealloc {
    NSLog(@"MKAEFilterByTofController销毁");
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"MK-TOF Filter";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEFilterByTofController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEFilterByTofModel alloc] init];
    }
    return _dataModel;
}
