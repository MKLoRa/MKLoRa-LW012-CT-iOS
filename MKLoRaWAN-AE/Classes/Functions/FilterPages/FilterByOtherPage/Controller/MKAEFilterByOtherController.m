...
#import "MKAEFilterByOtherController.h"
...
#import "MKAEFilterByRawDataCell.h"
#import "MKAEFilterEditSectionHeaderView.h"

#import "MKAEFilterByOtherModel.h"

@interface MKAEFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKAEFilterEditSectionHeaderViewDelegate,
MKAEFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKAEFilterByOtherModel *dataModel;

@end

@implementation MKAEFilterByOtherController

- (void)dealloc {
    NSLog(@"MKAEFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKAEFilterEditSectionHeaderView *headerView = [MKAEFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
        MKAEFilterEditSectionHeaderViewModel *model = [[MKAEFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKAEFilterByRawDataCell *cell = [MKAEFilterByRawDataCell initCellWithTableView:tableView];
...
#pragma mark - MKAEFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
- (void)mk_ae_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    if (index != 0) {
        return;
    }
    ...
    MKAEFilterByRawDataCellModel *cellModel = [[MKAEFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
/// 减号点击事件
/// @param index 所在index
- (void)mk_ae_filterEditSectionHeaderView_subButtonPressed:(NSInteger)index {
...
#pragma mark - MKAEFilterByRawDataCellDelegate
/// 输入框内容发生改变
/// @param textType 哪个输入框发生改变了
/// @param index 当前cell所在的row
/// @param textValue 当前textField内容
- (void)ae_rawFilterDataChanged:(mk_ae_filterRawAdvDataTextType)textType
                          index:(NSInteger)index
                      textValue:(NSString *)textValue {
    if (index >= self.section1List.count) {
        return;
    }
    MKAEFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_ae_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_ae_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_ae_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_ae_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKAEFilterByRawDataCellModel *cellModel = self.section1List[i];
        MKAEFilterRawAdvDataModel *model = [[MKAEFilterRawAdvDataModel alloc] init];
...
- (mk_ae_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_ae_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_ae_filterByOther_AB;
        }
        return mk_ae_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_ae_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_ae_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_ae_filterByOther_AOrBOrC;
        }
    }
    return mk_ae_filterByOther_AOrBOrC;
}
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Other Type Filter";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEFilterByOtherController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
