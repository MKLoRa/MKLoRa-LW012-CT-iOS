...
@interface MKAEFilterByMacController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEFilterByMacModel *dataModel;

@end

@implementation MKAEFilterByMacController

- (void)dealloc {
    NSLog(@"MKAEFilterByMacController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEFilterByMacController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEFilterByMacModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEFilterByMacModel alloc] init];
    }
    return _dataModel;
}
