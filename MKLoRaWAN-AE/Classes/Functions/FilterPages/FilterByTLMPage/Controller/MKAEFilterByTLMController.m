...
@interface MKAEFilterByTLMController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEFilterByTLMModel *dataModel;

@end

@implementation MKAEFilterByTLMController

- (void)dealloc {
    NSLog(@"MKAEFilterByTLMController销毁");
}
...
- (void)configFilterStatus:(BOOL)isOn {
    ...
    [MKAEInterface ae_configFilterByTLMStatus:isOn sucBlock:^{
        [[MKHudManager share] hide];
...
- (void)configTLMVersion:(NSInteger)version {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    [MKAEInterface ae_configFilterByTLMVersion:version sucBlock:^{
        [[MKHudManager share] hide];
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = @"Eddystone-TLM Filter";
...
- (MKAEFilterByTLMModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEFilterByTLMModel alloc] init];
    }
    return _dataModel;
}
