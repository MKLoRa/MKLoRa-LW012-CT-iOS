...
@interface MKAEGeneralController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEGeneralPageModel *dataModel;

@end

@implementation MKAEGeneralController

- (void)dealloc {
    NSLog(@"MKAEGeneralController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Device Mode
        MKAEDeviceModeController *vc = [[MKAEDeviceModeController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 1 && indexPath.row == 0) {
        //Auxiliary Operation
        MKAEAuxiliaryController *vc = [[MKAEAuxiliaryController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 0) {
        //Bluetooth Settings
        MKAEBleSettingsController *vc = [[MKAEBleSettingsController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 0) {
        //3-Axis Settings
        MKAEAxisSettingController *vc = [[MKAEAxisSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEGeneralController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEGeneralPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEGeneralPageModel alloc] init];
    }
    return _dataModel;
}
