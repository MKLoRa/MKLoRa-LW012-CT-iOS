...
@interface MKAEFilterByURLModel ()
...
@implementation MKAEFilterByURLModel
...
- (BOOL)readFilterStatus {
    ...
    [MKAEInterface ae_readFilterByURLStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configFilterStatus {
    __block BOOL success = NO;
    [MKAEInterface ae_configFilterByURLStatus:self.isOn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readURLContent {
    __block BOOL success = NO;
    [MKAEInterface ae_readFilterByURLContentWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configURLContent {
    __block BOOL success = NO;
    [MKAEInterface ae_configFilterByURLContent:self.url sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
