...
@interface MKAELoRaAppSettingModel ()
...
@implementation MKAELoRaAppSettingModel
...
- (BOOL)readTimeSyncInterval {
    ...
    [MKAEInterface ae_readLorawanTimeSyncIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeSyncInterval {
    __block BOOL success = NO;
    [MKAEInterface ae_configTimeSyncInterval:[self.timeSyncInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readNetworkCheckInterval {
    __block BOOL success = NO;
    [MKAEInterface ae_readLorawanNetworkCheckIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configNetworkCheckInterval {
    __block BOOL success = NO;
    [MKAEInterface ae_configLorawanNetworkCheckInterval:[self.checkInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
