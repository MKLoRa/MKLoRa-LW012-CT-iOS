//
//  MKAESelftestVoltageThresholdCell.m
...
@implementation MKAESelftestVoltageThresholdCellModel
@end

@interface MKAESelftestVoltageThresholdCell ()
...
@implementation MKAESelftestVoltageThresholdCell

+ (MKAESelftestVoltageThresholdCell *)initCellWithTableView:(UITableView *)tableView {
    MKAESelftestVoltageThresholdCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAESelftestVoltageThresholdCellIdenty"];
    if (!cell) {
        cell = [[MKAESelftestVoltageThresholdCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAESelftestVoltageThresholdCellIdenty"];
    }
    return cell;
}
...
- (void)selectedButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.thresholdList selectedRow:row block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ae_selftestVoltageThresholdCell_thresholdChanged:threshold:)]) {
            [self.delegate ae_selftestVoltageThresholdCell_thresholdChanged:self.dataModel.index threshold:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKAESelftestVoltageThresholdCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAESelftestVoltageThresholdCellModel.class] || _dataModel.threshold > 20) {
        return;
    }
