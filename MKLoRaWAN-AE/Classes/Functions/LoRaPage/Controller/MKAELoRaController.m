...
@interface MKAELoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKAELoRaPageModel *dataModel;

@end

@implementation MKAELoRaController

- (void)dealloc {
    NSLog(@"MKAELoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKAELoRaSettingController *vc = [[MKAELoRaSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.row == 2) {
        //Application Settings
        MKAELoRaAppSettingController *vc = [[MKAELoRaAppSettingController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
}
...
- (MKAELoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAELoRaPageModel alloc] init];
    }
    return _dataModel;
}
