...
@interface MKAEDeviceSettingModel ()
...
@implementation MKAEDeviceSettingModel
...
- (BOOL)readTimeZone {
    ...
    [MKAEInterface ae_readTimeZoneWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTimeZone {
    __block BOOL success = NO;
    [MKAEInterface ae_configTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPayload {
    __block BOOL success = NO;
    [MKAEInterface ae_readLowPowerPayloadStatusWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configLowPowerPayload {
    __block BOOL success = NO;
    [MKAEInterface ae_configLowPowerPayloadStatus:self.lowPowerPayload sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readLowPowerPayloadInterval {
    __block BOOL success = NO;
    [MKAEInterface ae_readLowPowerPayloadIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
