...
@interface MKAEDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_ae_centralManagerLogDelegate,
MKAEDebuggerCellDelegate>
...
@property (nonatomic, strong)MKAEDebuggerButton *deleteButton;

@property (nonatomic, strong)MKAEDebuggerButton *exportButton;
...
@implementation MKAEDebuggerController

- (void)dealloc {
    NSLog(@"MKAEDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ae_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_ae_startDebuggerMode" object:nil];
...
- (void)leftButtonMethod {
    if ([MKAECentralManager shared].connectStatus != mk_ae_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
    ...
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKAECentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKAEDebuggerCell *cell = [MKAEDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - MKAEDebuggerCellDelegate
- (void)ae_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKAEDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_ae_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_export_disableIcon.png");
    ...
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_ae_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKAELogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKAEDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKAELogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKAEDebuggerCellModel *cellModel = [[MKAEDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKAELogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKAEDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_delete_disableIcon.png");
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_ae_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKAEDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKAEDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_delete_disableIcon.png");
...
- (MKAEDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKAEDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-AE", @"MKAEDebuggerController", @"ae_export_disableIcon.png");
