...
@interface MKAEBleSettingsController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
mk_textSwitchCellDelegate,
MKAEBroadcastTxPowerCellDelegate>
...
@property (nonatomic, strong)MKAEBleSettingsModel *dataModel;
...
@implementation MKAEBleSettingsController

- (void)dealloc {
    NSLog(@"MKAEBleSettingsController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKAEBroadcastTxPowerCell *cell = [MKAEBroadcastTxPowerCell initCellWithTableView:tableView];
...
#pragma mark - MKAEBroadcastTxPowerCellDelegate
/*
 ...
 8,       //4dBm
 */
- (void)ae_txPowerValueChanged:(NSInteger)txPower {
    //Tx Power
    self.dataModel.txPower = txPower;
    MKAEBroadcastTxPowerCellModel *cellModel = self.section1List[0];
...
- (void)configPassword{
    ...
    [alertView showAlertWithTitle:@"Change Password" message:msg notificationName:@"mk_ae_needDismissAlert"];
}
...
- (void)setPasswordToDevice{
    ...
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKAEInterface ae_configPassword:password sucBlock:^{
        [[MKHudManager share] hide];
        [self.view showCentralToast:@"Success"];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKAEBroadcastTxPowerCellModel *cellModel = [[MKAEBroadcastTxPowerCellModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAEBleSettingsController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAEBleSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEBleSettingsModel alloc] init];
    }
    return _dataModel;
}
