...
@implementation MKAEReportTimePointCellModel
@end

@interface MKAEReportTimePointCell ()
...
@implementation MKAEReportTimePointCell

+ (MKAEReportTimePointCell *)initCellWithTableView:(UITableView *)tableView {
    MKAEReportTimePointCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAEReportTimePointCellIdenty"];
    if (!cell) {
        cell = [[MKAEReportTimePointCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAEReportTimePointCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(ae_cellDeleteButtonPressed:)]) {
        [self.delegate ae_cellDeleteButtonPressed:self.dataModel.index];
    }
}
...
- (void)hourButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.hourList selectedRow:selectedRow block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ae_hourButtonPressed:hour:)]) {
            [self.delegate ae_hourButtonPressed:self.dataModel.index hour:currentRow];
        }
    }];
}
...
- (void)timeSpaceButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.timeSpaceList selectedRow:selectedRow block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(ae_timeSpaceButtonPressed:timeSpace:)]) {
            [self.delegate ae_timeSpaceButtonPressed:self.dataModel.index timeSpace:currentRow];
        }
    }];
}
...
#pragma mark - setter
- (void)setDataModel:(MKAEReportTimePointCellModel *)dataModel {
    _dataModel = nil;
