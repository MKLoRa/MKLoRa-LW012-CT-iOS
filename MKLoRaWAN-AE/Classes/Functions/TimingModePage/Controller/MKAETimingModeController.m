...
@interface MKAETimingModeController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextButtonCellDelegate,
MKAETimingModeAddCellDelegate,
MKAEReportTimePointCellDelegate>
...
@property (nonatomic, strong)MKAETimingModeModel *dataModel;

@end

@implementation MKAETimingModeController

- (void)dealloc {
    NSLog(@"MKAETimingModeController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKAETimingModeAddCell *cell = [MKAETimingModeAddCell initCellWithTableView:tableView];
        cell.dataModel = self.section1List[indexPath.row];
        cell.delegate = self;
        return cell;
    }
    MKAEReportTimePointCell *cell = [MKAEReportTimePointCell initCellWithTableView:tableView];
...
#pragma mark - MKAETimingModeAddCellDelegate
- (void)ae_addButtonPressed {
    ...
    MKAEReportTimePointCellModel *cellModel = [[MKAEReportTimePointCellModel alloc] init];
...
#pragma mark - MKAEReportTimePointCellDelegate
/**
 删除
...
- (void)ae_cellDeleteButtonPressed:(NSInteger)index {
    ...
    for (NSInteger i = 0; i < self.section2List.count; i ++) {
        MKAEReportTimePointCellModel *cellModel = self.section2List[i];
        cellModel.index = i;
...
- (void)ae_hourButtonPressed:(NSInteger)index hour:(NSInteger)hour {
    ...
    MKAEReportTimePointCellModel *cellModel = self.section2List[index];
    cellModel.hour = hour;
}
...
- (BOOL)cellCanSelected{
    ...
    for (int i = 0; i < [arrCells count]; i++) {
        ...
        if ([cell isKindOfClass:MKAEReportTimePointCell.class]) {
            MKAEReportTimePointCell *tempCell = (MKAEReportTimePointCell *)cell;
...
- (void)loadSection2Datas {
    for (NSInteger i = 0; i < self.dataModel.pointList.count; i ++) {
        MKAETimingModeTimePointModel *tempModel = self.dataModel.pointList[i];
        MKAEReportTimePointCellModel *cellModel = [[MKAEReportTimePointCellModel alloc] init];
        cellModel.index = i;
        cellModel.msg = [NSString stringWithFormat:@"Time Point %ld",(long)(i + 1)];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-AE", @"MKAETimingModeController", @"ae_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKAETimingModeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAETimingModeModel alloc] init];
    }
    return _dataModel;
}
