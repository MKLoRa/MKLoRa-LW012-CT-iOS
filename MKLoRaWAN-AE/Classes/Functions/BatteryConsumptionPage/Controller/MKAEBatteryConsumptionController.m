//
//  MKAEBatteryConsumptionController.m
...
@interface MKAEBatteryConsumptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKAEBatteryConsumptionModel *dataModel;

@end

@implementation MKAEBatteryConsumptionController

- (void)dealloc {
    NSLog(@"MKAEBatteryConsumptionController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        MKAEBatteryInfoCell *cell = [MKAEBatteryInfoCell initCellWithTableView:tableView];
...
- (void)batteryReset {
    ...
    alertView.notificationName = @"mk_ae_needDismissAlert";
    @weakify(self);
...
- (void)sendBatteryResetCommandToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKAEInterface ae_batteryResetWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)loadSection0Datas {
    MKAEBatteryInfoCellModel *cellModel = [[MKAEBatteryInfoCellModel alloc] init];
...
- (MKAEBatteryConsumptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKAEBatteryConsumptionModel alloc] init];
    }
    return _dataModel;
}
