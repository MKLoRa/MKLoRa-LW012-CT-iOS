...
@implementation MKAELogDatabaseManager
...
+ (void)insertLogDatas:(NSArray <NSDictionary *>*)logList
    ...
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"MKAELogDB")];
    if (![db open]) {
        [self operationInsertFailedBlock:failedBlock];
        return;
    }
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists MKAELogDataTable (macAddress text,date text,logDetails text)"];
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKAELogDB")] inDatabase:^(FMDatabase *db) {
        
        for (NSDictionary *logDic in logList) {
            BOOL exist = NO;
            FMResultSet * result = [db executeQuery:@"select * from MKAELogDataTable where macAddress = ? and date = ?",macAddress,SafeStr(logDic[@"date"])];
            while (result.next) {
                exist = YES;
            }
            if (exist) {
                //存在该设备，更新设备
                [db executeUpdate:@"UPDATE MKAELogDataTable SET logDetails = ? where macAddress = ?",SafeStr(logDic[@"logDetails"]),macAddress];
            }else{
                //不存在，插入设备
                [db executeUpdate:@"INSERT INTO MKAELogDataTable (macAddress,date,logDetails) VALUES (?,?,?)",macAddress,SafeStr(logDic[@"date"]),SafeStr(logDic[@"logDetails"])];
            }
        }
...
+ (void)readLocalLogsWithMacAddress:(NSString *)macAddress
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKAELogDB")] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM MKAELogDataTable where macAddress = ?",macAddress];
        while ([result next]) {
            NSDictionary *dic = @{
...
+ (void)deleteDatasWithMacAddress:(NSString *)macAddress
                      ...
                      failedBlock:(void (^)(NSError *error))failedBlock {
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKAELogDB")] inDatabase:^(FMDatabase *db) {
        BOOL result = [db executeUpdate:@"DELETE FROM MKAELogDataTable where macAddress = ?",macAddress];
        if (!result) {
