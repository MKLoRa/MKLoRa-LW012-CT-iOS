...
@implementation MKAEFilterBeaconCellModel
@end

@interface MKAEFilterBeaconCell ()
...
@implementation MKAEFilterBeaconCell

+ (MKAEFilterBeaconCell *)initCellWithTableView:(UITableView *)tableView {
    MKAEFilterBeaconCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAEFilterBeaconCellIdenty"];
    if (!cell) {
        cell = [[MKAEFilterBeaconCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAEFilterBeaconCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKAEFilterBeaconCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAEFilterBeaconCellModel.class]) {
        return;
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ae_beaconMinValueChanged:index:)]) {
                [self.delegate mk_ae_beaconMinValueChanged:text index:self.dataModel.index];
            }
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(mk_ae_beaconMaxValueChanged:index:)]) {
                [self.delegate mk_ae_beaconMaxValueChanged:text index:self.dataModel.index];
            }
        };
    }
