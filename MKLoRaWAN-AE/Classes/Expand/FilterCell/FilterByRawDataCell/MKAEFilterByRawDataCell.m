...
@implementation MKAEFilterByRawDataCellModel
...
@interface MKAEFilterByRawDataCell ()
...
@implementation MKAEFilterByRawDataCell

+ (MKAEFilterByRawDataCell *)initCellWithTableView:(UITableView *)tableView {
    MKAEFilterByRawDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKAEFilterByRawDataCellIdenty"];
    if (!cell) {
        cell = [[MKAEFilterByRawDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKAEFilterByRawDataCellIdenty"];
    }
    return cell;
}
...
- (void)textFieldValueChanged:(NSString *)text textType:(mk_ae_filterRawAdvDataTextType)type {
    if ([self.delegate respondsToSelector:@selector(ae_rawFilterDataChanged:index:textValue:)]) {
        [self.delegate ae_rawFilterDataChanged:type index:self.dataModel.index textValue:text];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKAEFilterByRawDataCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKAEFilterByRawDataCellModel.class]) {
        return;
    }
...
- (MKTextField *)typeTextField {
    if (!_typeTextField) {
        ...
        _typeTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            ...
            [sself textFieldValueChanged:text textType:mk_ae_filterRawAdvDataTextTypeDataType];
        };
    }
...
- (MKTextField *)minTextField {
    if (!_minTextField) {
        ...
        _minTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_ae_filterRawAdvDataTextTypeMinIndex];
        };
    }
...
- (MKTextField *)maxTextField {
    if (!_maxTextField) {
        ...
        _maxTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_ae_filterRawAdvDataTextTypeMaxIndex];
        };
    }
...
- (MKTextField *)rawDataField {
    if (!_rawDataField) {
        ...
        _rawDataField.textChangedBlock = ^(NSString * _Nonnull text) {
            __strong typeof(self) sself = weakSelf;
            [sself textFieldValueChanged:text textType:mk_ae_filterRawAdvDataTextTypeRawDataType];
        };
    }
